...
class DCU_Net_Encoder(nn.Module):
    def __init__(self, conv_cfg, leaky_slope):
        ...
        self.conv = ComplexConvWrapper(nn.Conv2d, *conv_cfg, bias=True)
...
class DCU_Net_Decoder(nn.Module):
    def __init__(self, dconv_cfg, leaky_slope):
        ...
        self.dconv = ComplexConvWrapper(nn.ConvTranspose2d, *dconv_cfg, bias=True)
...
class DCGRU_Net_22(nn.Module):
    def __init__(self):
        ...
        # input [1, 3, x, 161] output [1, 2, x, 81]
        self.cnn_encoder1 = DCU_Net_Encoder([3, 2, [5, 7], [1, 2], [2, 3]], 0.1)
        self.rnn_encoder1 = GRU_Net_Encoder(2, 81)
        # input [1, 2, x, 81] output [1, 2, x, 81]
        self.cnn_encoder11 = DCU_Net_Encoder([2, 2, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 2, x, 81] output [1, 4, x, 41]
        self.cnn_encoder2 = DCU_Net_Encoder([2, 4, [5, 7], [1, 2], [2, 3]], 0.1)
        self.rnn_encoder2 = GRU_Net_Encoder(4, 41)
        # input [1, 4, x, 41] output [1, 4, x, 41]
        self.cnn_encoder22 = DCU_Net_Encoder([4, 4, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 4, x, 41] output [1, 8, x, 21]
        self.cnn_encoder3 = DCU_Net_Encoder([4, 8, [5, 7], [1, 2], [2, 3]], 0.1)
        self.rnn_encoder3 = GRU_Net_Encoder(8, 21)
        # input [1, 8, x, 21] output [1, 8, x, 21]
        self.cnn_encoder33 = DCU_Net_Encoder([8, 8, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 8, x, 21] output [1, 16, x, 11]
        self.cnn_encoder4 = DCU_Net_Encoder([8, 16, [3, 5], [1, 2], [1, 2]], 0.1)
        self.rnn_encoder4 = GRU_Net_Encoder(16, 11)
        # input [1, 16, x, 11] output [1, 16, x, 11]
        self.cnn_encoder44 = DCU_Net_Encoder([16, 16, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 16, x, 11] output [1, 32, x, 6]
        self.cnn_encoder5 = DCU_Net_Encoder([16, 32, [3, 5], [1, 2], [1, 2]], 0.1)
        self.rnn_encoder5 = GRU_Net_Encoder(32, 6)
        # input [1, 32, x, 6] output [1, 32, x, 6]
        self.cnn_encoder55 = DCU_Net_Encoder([32, 32, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 32, x, 6] output [1, 32, x, 3]
        self.cnn_encoder6 = DCU_Net_Encoder([32, 32, [3, 5], [1, 2], [1, 2]], 0.1)
        self.rnn_encoder6 = GRU_Net_Encoder(32, 3)
        # input [1, 32, x, 3] output [1, 32, x, 3]
        self.cnn_encoder66 = DCU_Net_Encoder([32, 32, [5, 7], [1, 1], [2, 3]], 0.1)
        # input [1, 32, x, 3] output [1, 32, x, 2]
        self.cnn_encoder7 = DCU_Net_Encoder([32, 32, [3, 5], [1, 2], [1, 2]], 0.1)
        self.rnn_encoder7 = GRU_Net_Encoder(32, 2)
        # input [1, 32, x, 2] output [1, 64, x, 2]
    ...
    def forward(self, input_real, input_imag):
        ...
        xr, xi = pad2d_as(xr, input_real[:, 2, :, :].unsqueeze(1)), pad2d_as(xi, input_imag[:, 2, :, :].unsqueeze(1))
        ratio_mask_fn = self.get_ratio_mask(xr, xi)
        return ratio_mask_fn(input_real[:, 2, :, :].unsqueeze(1), input_imag[:, 2, :, :].unsqueeze(1))
